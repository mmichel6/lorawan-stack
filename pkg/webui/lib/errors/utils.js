 ...
 * determining the type of the error.
 * @param {Object} error - The error to be tested.
 * @returns {number} The (clostest when grpc error) HTTP Status Code
 */
export const httpStatusCode = error => (
  isBackend(error)
    ? error.http_code || grpcErrToHttpErr(error.code || error.grpc_code) || 520 // Unknown Error
    : (Boolean(error) && error.statusCode) || 520 // Unknown Error
)
